...
export const createUser = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    await query(
      `INSERT INTO users (id, username, email, password, name, role, status, phone, created_at, updated_at, created_by) 
...
export const listUsers = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const result = await query(
      `SELECT id, username, email, name, role, status, phone, created_at, updated_at, created_by
       FROM users 
       ${whereClause}
...
export const updateUser = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    // Atualizar no banco
    await query(
      `UPDATE users SET ${updates.join(', ')}, updated_by = $${paramIndex - 1} WHERE id = $${paramIndex}`,
      values
    );
...
export const resetUserPassword = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    await query(
      ...
      [hashedPassword, new Date().toISOString(), req.user!.id, id]
    );
...
export const deactivateUser = async (req: AuthRequest, res: Response): Promise<Response> => {
  try {
    ...
    await query(
      ...
      ['inactive', new Date().toISOString(), req.user!.id, id]
    );
