...
exports.deactivateTherapist = exports.updateTherapist = exports.listTherapists = exports.createTherapist = void 0;
...
const createTherapist = async (req, res) => {
    try {
        ...
        const { email, password, name, phone, cpf, professional_id, specialties = [], bio, experience_years, languages = ['Português'], available_hours = {}, consultation_duration = 50, max_daily_appointments = 8 } = req.body;
        // Validações básicas
        ...
        const existingUser = await (0, database_1.query)('SELECT id FROM users WHERE email = ?', [email.toLowerCase()]);
        ...
        const therapistId = (0, uuid_1.v4)();
        // Criar usuário
        await (0, database_1.query)('INSERT INTO users (id, email, password, name, role, status, phone, cpf, created_at, updated_at, created_by) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            userId,
            email.toLowerCase(),
            hashedPassword,
            name,
        ...
        await (0, database_1.query)('INSERT INTO therapists (id, user_id, professional_id, specialties, bio, experience_years, languages, available_hours, consultation_duration, max_daily_appointments, active, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)', [
            therapistId,
            userId,
...
exports.listTherapists = listTherapists;
// Atualizar terapeuta
const updateTherapist = async (req, res) => {
    try {
        ...
        const { name, phone, cpf, professional_id, specialties, bio, experience_years, languages, available_hours, consultation_duration, max_daily_appointments, active } = req.body;
        // Verificar se terapeuta existe
        const therapistResult = await (0, database_1.query)('SELECT t.*, u.id as user_id FROM therapists t JOIN users u ON t.user_id = u.id WHERE t.id = ?', [therapistId]);
        ...
        if (userUpdates.length > 0) {
            ...
            await (0, database_1.query)(`UPDATE users SET ${userUpdates.join(', ')} WHERE id = ?`, userValues);
        }
        ...
        if (therapistUpdates.length > 0) {
            ...
            await (0, database_1.query)(`UPDATE therapists SET ${therapistUpdates.join(', ')} WHERE id = ?`, therapistValues);
        }
...
const deactivateTherapist = async (req, res) => {
    try {
        ...
        const { therapistId } = req.params;
        await (0, database_1.query)('UPDATE therapists SET active = ?, updated_at = ? WHERE id = ?', [false, new Date().toISOString(), therapistId]);
        await (0, database_1.query)('UPDATE users SET status = ?, updated_at = ? WHERE id = (SELECT user_id FROM therapists WHERE id = ?)', ['inactive', new Date().toISOString(), therapistId]);
...
exports.deactivateTherapist = deactivateTherapist;
