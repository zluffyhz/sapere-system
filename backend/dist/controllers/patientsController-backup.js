...
const getPatients = async (req, res) => {
    try {
        ...
        if (diagnosis) {
            whereClause += ` AND p.tags @> ARRAY[$${paramIndex}]`;
            params.push(diagnosis.toString());
            paramIndex++;
        }
        if (active !== undefined) {
...
const createPatient = async (req, res) => {
    try {
        ...
        const { name, social_name, email, phone, birth_date, cpf, rg, gender, address, diagnosis, medications, allergies, special_needs, school_info, work_info, responsible_users, emergency_contacts, general_notes, internal_notes } = req.body;
    ...
    `, [
        ...
        const result = await (0, database_1.query)('SELECT * FROM patients WHERE id = ?', [patientId]);
...
const updatePatient = async (req, res) => {
    try {
        ...
        const checkResult = await (0, database_1.query)('SELECT id FROM patients WHERE id = ?', [patientId]);
...
const deactivatePatient = async (req, res) => {
    try {
        ...
        await (0, database_1.query)('UPDATE patients SET active = 0, updated_at = ?, updated_by = ? WHERE id = ?', [currentTime, user.id, patientId]);
        res.json({
...
const getPatientsStats = async (req, res) => {
    try {
        ...
        const activeResult = await (0, database_1.query)('SELECT COUNT(*) as total FROM patients WHERE active = 1');
        // Buscar pacientes para calcular estatísticas detalhadas
        const patientsResult = await (0, database_1.query)('SELECT birth_date, diagnosis FROM patients WHERE active = 1');
        ...
        const stats = {
            ...
            by_diagnosis: patients.reduce((acc, patient) => {
...
//# sourceMappingURL=patientsController-backup.js.map