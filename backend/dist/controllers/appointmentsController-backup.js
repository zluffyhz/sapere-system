...
exports.getAppointmentsStats = exports.completeAppointment = exports.cancelAppointment = exports.confirmAppointment = exports.updateAppointment = exports.createAppointment = exports.getAppointment = exports.getAppointments = void 0;
...
const getAppointments = async (req, res) => {
    try {
        const user = req.user;
        const { patient_id, therapist_id, status, start_date, end_date, upcoming_only } = req.query;
        let filteredAppointments = APPOINTMENTS;
        // Filtros de permissão por role
        if (user?.role === 'therapist') {
            filteredAppointments = filteredAppointments.filter(apt => apt.therapist_id === user.id);
        }
            ...
            filteredAppointments = filteredAppointments.filter(apt => userPatients.includes(apt.patient_id));
        }
        // Aplicar filtros
        if (patient_id) {
            filteredAppointments = filteredAppointments.filter(apt => apt.patient_id === patient_id);
        }
        if (therapist_id) {
            filteredAppointments = filteredAppointments.filter(apt => apt.therapist_id === therapist_id);
        }
        if (status) {
            filteredAppointments = filteredAppointments.filter(apt => apt.status === status);
        }
        if (start_date) {
            filteredAppointments = filteredAppointments.filter(apt => new Date(apt.appointment_date) >= new Date(start_date));
        }
        if (end_date) {
            filteredAppointments = filteredAppointments.filter(apt => new Date(apt.appointment_date) <= new Date(end_date));
        }
        if (upcoming_only === 'true') {
            const now = new Date();
            filteredAppointments = filteredAppointments.filter(apt => new Date(apt.appointment_date) > now && apt.status !== 'cancelled');
        }
        // Ordenar por data
...
const createAppointment = async (req, res) => {
    try {
        ...
        const conflictingAppointment = APPOINTMENTS.find(apt => apt.therapist_id === appointmentData.therapist_id &&
...
const getAppointmentsStats = async (req, res) => {
    try {
        ...
        const stats = {
            ...
            upcoming: filteredAppointments.filter(apt => new Date(apt.appointment_date) > now && apt.status !== 'cancelled').length,
...
//# sourceMappingURL=appointmentsController-backup.js.map