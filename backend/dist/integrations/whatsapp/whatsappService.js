...
exports.SAPERE_WHATSAPP_TEMPLATES = exports.WhatsAppService = void 0;
...
class WhatsAppService {
    ...
    async updateMessageStatus(status) {
        try {
            ...
            await (0, database_1.query)(`UPDATE communications 
         SET status = $1, ${updateField} = $2, provider_response = $3, updated_at = NOW()
         WHERE provider_message_id = $4`, [dbStatus, timestamp, JSON.stringify(status), status.id]);
           ...
           WHERE provider_message_id = $3`, [errorMessage, status.errors[0]?.code.toString(), status.id]);
            }
        }
        catch (error) {
     ...
     */
    async handleAppointmentConfirmation(phone, confirmed) {
        try {
      ...
      `, [cleanPhone]);
            if (result.rows.length > 0) {
             ...
             WHERE id = $1`, [appointment.id]);
                    // Enviar mensagem de confirmação recebida
                    await this.sendText(phone, `✅ Consulta confirmada! Obrigado, ${appointment.patient_name}. ` +
             ...
             WHERE id = $1`, [appointment.id]);
                    ...
                    await this.sendText(phone, `📅 Entendido! Vamos remarcar sua consulta. ` +
     ...
     * Registrar mensagem recebida no histórico
     */
    async logIncomingMessage(message, contact) {
        try {
            const cleanPhone = this.formatPhoneNumber(message.from);
            // Buscar paciente pelo telefone
            const patientResult = await (0, database_1.query)('SELECT id, name FROM patients WHERE phone = $1', [cleanPhone]);
            let patientId = null;
     ...
     */
    mapWhatsAppStatusToDbStatus(status) {
        switch (status) {
            case 'sent': return communication_1.CommunicationStatus.SENT;
            case 'delivered': return communication_1.CommunicationStatus.DELIVERED;
            case 'read': return communication_1.CommunicationStatus.READ;
            case 'failed': return communication_1.CommunicationStatus.FAILED;
            default: return communication_1.CommunicationStatus.SENT;
        }
    }
    /**
...
exports.SAPERE_WHATSAPP_TEMPLATES = {
