...
class EmailService {
     ...
     */
    async sendTemplateEmail(to, templateType, variables, attachments) {
        try {
     ...
     */
    async sendTherapyReport(to, patientName, reportData, pdfAttachment) {
        try {
     ...
     */
    async sendNewsletter(recipients, title, content, imageUrls) {
        const results = {
            success: true,
            sent: 0,
            failed: 0,
        ...
        for (const recipient of recipients) {
            try {
                ...
                const result = await this.sendEmail(recipient, `Newsletter Sapere - ${title}`, htmlContent);
                if (result.success) {
                    results.sent++;
                }
                else {
    ...
    async getEmailTemplate(templateType) {
        try {
            const result = await (0, database_1.query)('SELECT * FROM communication_templates WHERE type = $1 AND channel = $2 AND active = true', [templateType, 'email']);
    ...
    getDefaultEmailTemplate(templateType) {
        const templates = {
            [communication_1.MessageTemplate.APPOINTMENT_CONFIRMATION]: {
            ...
            [communication_1.MessageTemplate.APPOINTMENT_REMINDER_24H]: {
            ...
            [communication_1.MessageTemplate.APPOINTMENT_CANCELLED]: {
            ...
            [communication_1.MessageTemplate.WELCOME_MESSAGE]: {
     ...
     */
    generateTherapyReportHTML(patientName, reportData) {
        return `
    <!DOCTYPE html>
    <html>
    <head>
     ...
     */
    generateNewsletterHTML(title, content, imageUrls) {
